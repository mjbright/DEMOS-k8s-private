...
BACKEND_SVC='flask-db'
BACKEND_PORT=9000
BACKEND_URL=f'http://{BACKEND_SVC}:{BACKEND_PORT}'
...
def root():
    ...
    resp_text=f"Received request to '/' - forwarding to { BACKEND_URL }\n"
...
def catch_all(path):
    ...
    resp_text=f"UNHANDLED PATH: Received request to '/{path} - forwarding to { BACKEND_URL }/{path}\n"
    return requests.get(f'{BACKEND_URL}/{path}').content
